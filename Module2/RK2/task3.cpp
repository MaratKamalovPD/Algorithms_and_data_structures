...
struct Node
{
	...
	{
		return ((product_name == _node.product_name) && (color == _node.color) && (size == _node.size));
	}
};


struct HasherForNode
{
	unsigned int operator()(const Node& _value, int i) const
	{
		unsigned int color_hash = ((_value.color >> 16) ^ _value.color) * 0x45d9f3b;
		unsigned int size_hash = ((_value.size >> 16) ^ _value.size) * 0x119de1f3;
		unsigned int product_name_hash = 0;
		for (unsigned int j = 0; j < _value.product_name.size(); ++j) // считаем h2 методом Горнера с проходом от начала строки к концу
		{
			product_name_hash = product_name_hash * 877 + int(_value.product_name[j]);
		}
		return (color_hash + size_hash + product_name_hash) * i * 2 + 1;
...
void Run(std::istream& input, std::ostream& output)
{
	...
	HasherForNode hasher;
	HashTable<Node, HasherForNode> table(8, hasher);
	...
	while (input >> operation >> node.product_name >> node.color >> node.size)
	{
		...
		{
		case '+':
			output << (table.Add(node) ? "OK" : "FAIL") << std::endl;
			break;
		case '-':
			output << (table.Delete(node) ? "OK" : "FAIL") << std::endl;
			break;
		case '?':
			output << (table.Has(node) ? "OK" : "FAIL") << std::endl;
			break;
		default:
			assert(false);
		}
